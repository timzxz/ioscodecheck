...
# 1 下载工程失败
# 2 有gemfile但是bundler install 失败
# 3 安装p12证书失败
# 4 安装描述文件失败
# 5 pod update失败
# 6 xcodebuild 失败
# 7 xcodebuild export失败
# 10 上传产物失败
...
function download_repository() {
    ...
    if [[ ! -d "$clone_code_dir" ]]; then
        ...
        log=$(git clone -b "${branch}" --depth=1 "${repo_url}" "$clone_code_dir"
            cd "$clone_code_dir")
    else 
        cd "$clone_code_dir"
...
# 配置测试ssh，url需要动态下发
# 生成命令：ssh-keygen -t rsa -P ''
function update_ssh_key(){
	rm -rf ~/.ssh && mkdir ~/.ssh
	wget $GIT_SSH_URL -O ~/.ssh/id_rsa 
	# 避免yes确认
	...
	if [[ "$GIT_ACCOUNT_NAME" != "" && "$GIT_ACCOUNT_EMAIL" != "" ]];then
		echo "[INFO] set GIT_ACCOUNT_NAME $GIT_ACCOUNT_NAME"
		echo "[INFO] set GIT_ACCOUNT_EMAIL $GIT_ACCOUNT_EMAIL"
...
function bundler_env() {
    ...
    if [[ -f "${gemfile_path}/Gemfile" ]];then
        ...
        echo "CLOUD_BUILD_ERROR_LOG: 不存在 ${gemfile_path}/Gemfile 文件"
        return 1
    fi
    ...
    ret=$(bundle install --path=`pwd` 2>&1)
    if [ $? != 0 ]; then
        exit 2
    fi
...
function cert_install() {
    ...
    if [[ ! "${cert_url}" == "" ]]; then
        ...
        ./bigMac utility install-p12 cert.p12 ${cert_passwd} ${login_passwd} 2>&1
    else
        echo "CLOUD_BUILD_WARNING_LOG: 未传入证书, 退出"
        exit 3
    fi
...
function provision_install() {
    echo "CLOUD_BUILD_INFO_LOG: -----------------------"
    echo "CLOUD_BUILD_INFO_LOG: 执行安装描述文件"

    if [[ ! "${prov_url}" == "" ]]; then
        PROV_PATH="${PROJECT_PATH}/Provisions"
        if [ ! -d ${PROV_PATH} ]; then
            mkdir -p ${PROV_PATH}
        fi
        cd ${PROV_PATH}
        curl -o prov.mobileprovision ${prov_url}
    else 
        echo "CLOUD_BUILD_WARNING_LOG: 未传入描述文件, 退出"
        exit 4
    fi
    echo $(ls ${PROV_PATH})
    cd ~
...
function pod_update() {
    ...
    if [ "${findGemfile}" == "1" ]; then
        ## 2.  打印 cocoapods 版本
        bundle exec pod --version

        ## 3. pod 依赖下载
            # 执行默认，同时对当前Podfile里面的源进行update以避免一些时序问题导致版本拉不到
        ret=$(bundle exec pod update --verbose 2>&1)
    else 
        ret=$(pod update)
    fi
    if [ $? != 0 ]; then
...
function xcodebuild_export() {
    ...
    echo "xcodebuild_path:${xcodebuild_path} xcodebuild_export_command:${xcodebuild_export_command}"
...
function _upload_file() {
    ...
    local response=$(curl --retry 5 --location --request POST ${UPLOAD_FILE_HOST}/inner/base/upload \
    --form file=@$file \
    --form filepath=cloud_build_output/${RECORD_ID}/${file_md5})
    echo $response
    local output_url=$(echo $response | jq ".data[]")
    if [[ -z output_url ]];then
        echo "[ERROR] fail to upload $output_file"
        exit 10
    fi
    ...
    artifacts="$artifacts$seperate$file_model"
    seperate=","
...
function save_artifacts() {
    ...
    for index in $(seq 0 $artifacts_len); do
        ...
        artifacts_path="${TARGETCODEPATH}/products/${artifacts_path}"
        ...
        if [[ ! -z $is_exist ]]; then
            ...
            for file_path in ${is_exist[@]}; do
                ...
                if [[ -f ${file_path} ]]; then
                    ...
                    _upload_file "${file_path}" ${file_name}
                    echo "upload end _upload_file"
                    if [[ ${upload_result} != "success" ]]; then
                        echo "upload artifacts $file_name  fail"
    ...
    artifacts="$artifacts]"
    response=$(curl --retry 5 --location --request POST ${UPLOAD_ARTIFACT_HOST}/client/upload_artifact \
    --form record_id=${RECORD_ID} \
    --form artifacts="${artifacts}")
    local result=$(echo $response | jq ".error_no")
    if [[ result -ne 0 ]];then
        echo "[ERROR] fail to notify upload event"
        exit 10
    fi
...
function upload_artifacts() {
    ...
    artifacts="["
    artifacts_path='[{"path":"*.ipa","type": "ipa"},{"path":"*.dSYM*","type": "dsym"}]'
...
if [ ! -d "${TARGETCODEPATH}/Project/ExportPlists/" ]; then
    mkdir -p "${TARGETCODEPATH}/Project/ExportPlists/"
fi
export_option_plist_path="${TARGETCODEPATH}/Project/ExportPlists/ExportOptions.plist"
xcodebuild_export
#step 8 
collect_data
#step 9
